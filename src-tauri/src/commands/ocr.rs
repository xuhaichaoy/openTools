...
const OCR_REC_MODEL_CANDIDATES: &[&str] = &[
    // Prefer higher-accuracy / newer recognizers first.
...
pub struct OcrRuntimeInfo {
    ...
    pub runtime_library_error: Option<String>,
    pub runtime_search_dirs: Vec<String>,
    pub ready: bool,
}
...
fn current_dir_candidates() -> Vec<PathBuf> {
    if let Ok(cwd) = std::env::current_dir() {
        return vec![
            cwd.join("src-tauri").join("resources").join("ocr"),
            cwd.join("src-tauri")
                .join("resources")
                .join("models")
                .join("ppocr"),
            cwd.join("resources").join("ocr"),
            cwd.join("resources").join("models").join("ppocr"),
        ];
    }
    vec![]
}
...
fn runtime_search_dirs(app: &AppHandle) -> Vec<PathBuf> {
    let mut dirs = Vec::new();

    if let Ok(resource_dir) = app.path().resource_dir() {
        dirs.push(resource_dir.clone());
        dirs.push(resource_dir.join("lib"));
        dirs.push(resource_dir.join("ocr"));
        dirs.push(resource_dir.join("models"));
        dirs.push(resource_dir.join("models").join("ppocr"));
    }

    if let Ok(app_data_dir) = app.path().app_data_dir() {
        dirs.push(app_data_dir.clone());
        dirs.push(app_data_dir.join("lib"));
        dirs.push(app_data_dir.join("models"));
        dirs.push(app_data_dir.join("models").join("ppocr"));
    }
    ...
    if let Ok(cwd) = std::env::current_dir() {
        ...
        dirs.push(cwd.join("src-tauri").join("resources").join("lib"));
        dirs.push(cwd.join("src-tauri").join("resources").join("ocr"));
        dirs.push(cwd.join("resources"));
        dirs.push(cwd.join("resources").join("lib"));
        dirs.push(cwd.join("resources").join("ocr"));
    }
...
fn runtime_library_candidates(app: &AppHandle) -> Vec<PathBuf> {
    ...
    for dir in runtime_search_dirs(app) {
        for file_name in &file_names {
...
fn ensure_ort_runtime(app: &AppHandle) -> Result<Option<String>, String> {
    let candidates = runtime_library_candidates(app);
    if candidates.is_empty() {
        let expected = runtime_library_file_names().join(", ");
        let searched = runtime_search_dirs(app)
            .iter()
    ...
    for candidate in candidates {
        match ort::init_from(candidate.to_string_lossy().to_string()).commit() {
            Ok(_) => {
...
fn model_dirs(app: &AppHandle) -> Vec<PathBuf> {
    ...
    dirs.extend(current_dir_candidates());
    dedup_dirs(dirs)
}

fn find_model_path(app: &AppHandle, file_name: &str) -> Option<PathBuf> {
    model_dirs(app)
        .into_iter()
...
fn collect_rec_model_candidates(app: &AppHandle) -> Vec<PathBuf> {
...
fn find_any_rec_model_path(app: &AppHandle) -> Option<PathBuf> {
    collect_rec_model_candidates(app).into_iter().next()
}

fn collect_ocr_keys_candidates(app: &AppHandle) -> Vec<PathBuf> {
    let dirs = model_dirs(app);
    let mut paths = Vec::new();
...
fn find_ocr_keys_path(app: &AppHandle, expected_class_count: Option<usize>) -> Option<PathBuf> {
    let candidates = collect_ocr_keys_candidates(app);
    if candidates.is_empty() {
        return None;
    }
    ...
    if let Some(classes) = expected_class_count {
        for path in &candidates {
            ...
            // CTC 识别字典需要额外包含 blank 和 space 两个位置
...
fn preferred_install_dir(app: &AppHandle) -> Result<PathBuf, String> {
    app.path()
        .app_data_dir()
        .map(|dir| dir.join("models").join("ppocr"))
...
fn otsu_binary_image(img_src: &RgbImage) -> RgbImage {
    ...
    // PaddleOCR 更偏向深色文字+浅色背景；若当前是深底浅字则反色一次
    if mean < 128.0 {
...
/// 从 ONNX 模型输入节点提取固定的 (height, width) 尺寸
/// 返回 Some((h, w)) 表示模型要求固定尺寸，None 表示支持动态尺寸
fn extract_fixed_input_size(input: &ort::session::Input) -> Option<(u32, u32)> {
    if let ort::value::ValueType::Tensor { dimensions, .. } = &input.input_type {
        // shape 格式: [batch, channels, height, width]，动态维度为 -1
        if dimensions.len() >= 4 {
            let h = dimensions[2];
            let w = dimensions[3];
            if h > 0 && w > 0 {
...
/// 从 ONNX 模型输出节点提取类别数（通常是最后两维中的较大者）
fn extract_rec_class_count(output: &ort::session::Output) -> Option<usize> {
    if let ort::value::ValueType::Tensor { dimensions, .. } = &output.output_type {
        let mut tail_dims = dimensions
            .iter()
            .rev()
...
/// 从识别输出节点估算时间步长（CTC 序列长度）。
/// 常见输出形状: [N, T, C] 或 [N, C, T]。
fn extract_rec_step_count(
    output: &ort::session::Output,
...
impl OcrEngine {
    pub fn new(app: &AppHandle) -> Result<Self, String> {
        ...
        let runtime_path = ensure_ort_runtime(app)?;
        let runtime_ms = runtime_started.elapsed().as_millis();
        if let Some(path) = runtime_path {
            log::info!("OCR runtime loaded from {:?}", path);
        }

        let det_model_path = find_model_path(app, "ppocr_det.onnx")
            .ok_or_else(|| "缺少模型文件: ppocr_det.onnx".to_string())?;
        let rec_model_candidates = collect_rec_model_candidates(app);
        ...
        let det_input_name = det_input.name.clone();

        // 从模型元数据提取固定输入尺寸（若有）
        let det_fixed_size = extract_fixed_input_size(det_input);
        if let Some((h, w)) = det_fixed_size {
            log::info!("OCR 检测模型使用固定输入尺寸: {}x{}", w, h);
        } else {
            log::info!("OCR 检测模型使用动态输入尺寸");
        }
        ...
        for rec_model_path in rec_model_candidates {
            ...
            let rec_input_name = rec_input.name.clone();
            ...
            let Some(keys_path) = find_ocr_keys_path(app, rec_class_count) else {
                rec_model_errors.push(format!(
                    "{} (找不到匹配字典 classes={:?})",
                    rec_model_path.to_string_lossy(),
                    rec_class_count
        ...
        log::info!("OCR 识别模型路径: {:?}", rec_model_path);
        if let Some(w) = rec_fixed_width {
            log::info!("OCR 识别模型使用固定输入宽度: {}", w);
            log::warn!("OCR 当前未加载到动态宽度识别模型，将使用固定宽度路径");
        } else {
            log::info!("OCR 识别模型使用动态输入宽度");
        }
        if let Some(classes) = rec_class_count {
            log::info!("OCR 识别模型输出类别数: {}", classes);
        } else {
            log::info!("OCR 识别模型输出类别数: 动态/未知");
        }
        if let Some(steps) = rec_step_count {
            log::info!("OCR 识别模型输出序列步长: {}", steps);
        }
        log::info!("OCR 使用字符字典: {:?}", keys_path);
        if let Some(classes) = rec_class_count {
    ...
    fn rec_retry_thresholds(&self) -> (f32, f32, f32) {
        // Large-char-dict models (e.g. PP-OCRv5 with ~18k classes) produce
        // lower softmax peak confidence; use gentler retry thresholds to
        // avoid triggering expensive multi-pass retries for almost every line.
    ...
    fn run_rec_on_prepared(&self, prepared: &RgbImage) -> Result<(String, f32), String> {
        let input_tensor = subtract_mean_normalize(prepared, &OCR_REC_MEAN, &OCR_REC_NORM);
        let outputs = self
            .rec_session
            .run(inputs![self.rec_input_name.clone() => input_tensor].map_err(|e| e.to_string())?)
    ...
    fn merge_rec_segments(merged: &mut String, segment: &str) {
        ...
        if overlap >= 8 {
            log::info!("OCR merge overlap chars: {}", overlap);
        } else {
            log::debug!("OCR merge overlap chars: {}", overlap);
        }
    ...
    fn recognize_text_line_once(&self, img_src: &RgbImage) -> Result<(String, f32), String> {
    ...
    fn recognize_text_line(&self, img_src: &RgbImage) -> Result<(String, f32), String> {
        ...
        // Retry only when text quality is weak or confidence is truly low.
        // This avoids very expensive multi-pass retries on good lines.
    ...
    fn detect_boxes(&self, img_src: &RgbImage) -> Result<Vec<TextBox>, String> {
        ...
        let outputs = self
            .det_session
            .run(inputs![self.det_input_name.clone() => input_tensor].map_err(|e| e.to_string())?)
        ...
        let pred_data = tensor.iter().copied().collect::<Vec<_>>();
        ...
        let after = boxes.len();
        if after < before {
            log::info!("OCR text box NMS reduced {} -> {}", before, after);
        }
    ...
    fn detect_once(&self, img_src: &RgbImage) -> Result<Vec<OcrBlock>, String> {
        ...
        for text_box in text_boxes {
            ...
            for line_img in &split_lines {
                ...
                let (text, confidence) = match self.recognize_text_line(line_img) {
                    Ok(result) => result,
                    Err(err) => {
                        log::warn!("OCR line recognition failed: {}", err);
                        continue;
                    }
                };
...
pub async fn ocr_detect_advanced(
    ...
    // 解码 base64 图片
    let b64_decode_started = Instant::now();
    let image_data = BASE64
        .decode(&image_base64)
        .map_err(|e| format!("Invalid base64 image: {}", e))?;
    let b64_decode_ms = b64_decode_started.elapsed().as_millis();
    ...
    match catch_unwind(AssertUnwindSafe(|| {
        ...
        let result = engine.detect_text(&image_data, &lang, detect_rotation, merge_paragraph)?;
        let detect_ms = detect_started.elapsed().as_millis();
        log::info!(
            "OCR command timing: base64_decode={}ms, engine_init={}ms, detect={}ms, inner_total={}ms",
            b64_decode_ms,
            init_ms,
            detect_ms,
        ...
        Ok(result) => {
            if result.is_ok() {
                log::info!(
                    "OCR command total elapsed: {}ms",
                    command_started.elapsed().as_millis()
                );
            }
            result
        }
        Err(payload) => Err(format!("OCR runtime panic: {}", panic_message(payload))),
    }
}
...
pub async fn ocr_list_models(app: AppHandle) -> Result<Vec<OcrModelInfo>, String> {
    let models = vec![
        OcrModelInfo {
            ...
            installed: find_model_path(&app, "ppocr_rec.onnx").is_some(),
            file_size_mb: 12.0,
        },
        OcrModelInfo {
            ...
            installed: find_model_path(&app, "ppocr_rec_en.onnx").is_some(),
            file_size_mb: 8.0,
        },
        OcrModelInfo {
            ...
            installed: find_model_path(&app, "ppocr_rec_ja.onnx").is_some(),
            file_size_mb: 10.0,
        },
        OcrModelInfo {
            ...
            installed: find_model_path(&app, "ppocr_rec_ko.onnx").is_some(),
            file_size_mb: 10.0,
        },
    ];

    // 检测模型和方向分类模型（通用）
    let det_installed = find_model_path(&app, "ppocr_det.onnx").is_some();
    let cls_installed = find_model_path(&app, "ppocr_cls.onnx").is_some();
    let keys_installed = find_ocr_keys_path(&app, None).is_some();
    ...
    let mut result = vec![
        ...
        OcrModelInfo {
            language: "keys".to_string(),
            display_name: "字符字典 (通用)".to_string(),
            file_name: "ppocr_keys_v5.txt (推荐)".to_string(),
...
#[tauri::command]
pub async fn ocr_get_runtime_info(app: AppHandle) -> Result<OcrRuntimeInfo, String> {
    let install_dir = preferred_install_dir(&app)?;
    let search_dirs = model_dirs(&app);
    let required_files = vec![
        "ppocr_det.onnx".to_string(),
        "ppocr_rec*.onnx (任一，优先动态宽度/高精度)".to_string(),
        format!("{} (任一)", OCR_KEYS_FILE_CANDIDATES.join(" / ")),
    ];
    let mut missing_required_files = Vec::new();
    if find_model_path(&app, "ppocr_det.onnx").is_none() {
        missing_required_files.push("ppocr_det.onnx".to_string());
    }
    if find_any_rec_model_path(&app).is_none() {
        missing_required_files.push("ppocr_rec*.onnx".to_string());
    }
    if find_ocr_keys_path(&app, None).is_none() {
        missing_required_files.push(OCR_KEYS_FILE_CANDIDATES[0].to_string());
    }
    let runtime_search_dirs = runtime_search_dirs(&app)
        .iter()
        .map(|dir| dir.to_string_lossy().to_string())
        .collect::<Vec<_>>();
    let (runtime_library_loaded, runtime_library_path, runtime_library_error) =
        match ensure_ort_runtime(&app) {
            Ok(path) => (true, path, None),
            Err(e) => (false, None, Some(e)),
        };
    let ready = missing_required_files.is_empty() && runtime_library_loaded;
...
#[tauri::command]
pub async fn ocr_open_model_dir(app: AppHandle) -> Result<String, String> {
    let install_dir = preferred_install_dir(&app)?;
    std::fs::create_dir_all(&install_dir).map_err(|e| format!("Create dir failed: {}", e))?;
    open::that(&install_dir).map_err(|e| format!("Open dir failed: {}", e))?;
