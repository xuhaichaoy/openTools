...
pub fn get_system_prompt(
    ...
    let mut prompt = if enable_advanced {
        format!(
             ...
             本地媒体回传规则（必须遵守）：\n\
             - 当用户说“发我图 / 把图片给我 / 把文件发我 / 直接给我看”时，如果你已经拿到本地图片或文件的绝对路径，不要调用 open_path。\n\
             - 正确做法：正文正常说明结果，然后单独输出一行 MEDIA:<绝对路径或 URL>。\n\
             - 桌面端会直接渲染 MEDIA: 指向的本地图片或文件；不要只输出文件名，也不要只说“路径在 /Users/...”。\n\
             - 一条回复里可以有多条 MEDIA:，每条单独占一行。\n\
             - 只有当用户明确要求“用系统打开 / 在 Finder 中打开 / 用默认程序打开”时，才使用 open_path。\n\
             - 如果路径不确定，先用 get_system_info 获取 home/downloads 等绝对目录，再拼出正确路径。\n\n\
