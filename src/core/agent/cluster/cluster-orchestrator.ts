...
请分析任务并返回 JSON 执行计划。只返回 JSON，不要附加其他内容。`;
  ...
  private async executeStep(
    ...
    try {
      ...
      task = `${autonomyHint}${projectBlock}\n\n## 任务\n${task}`;
  ...
  private async reviewStepWithTools(
    ...
    const bridge = new LocalAgentBridge(instanceId);
    this.bridges.set(instanceId, bridge);

    const reviewTask = `[集群自主模式] 你正在 Agent 集群中作为代码审查员执行，无法与用户交互。
...
## 审查流程
1. 从执行结果中提取被修改的文件路径
2. 用 read_file_range 读取这些文件的实际内容，验证修改是否正确
3. 用 search_in_files 检查是否有遗漏的引用需要更新
4. 检查代码逻辑、边界条件、错误处理
...
只返回 JSON，不要附加其他内容。`;
  ...
  private async aggregatePhase(
    ...
    const CODER_BUDGET = 20000; // coder 步骤给更大空间，保留代码细节
