...
export const AI_PRODUCT_MODE_DEFINITIONS: Record<AIProductMode, AIProductModeDefinition> = {
  ...
  review: {
    ...
    runtimeMode: "dialog",
    aiCenterMode: "review",
    boundaryHeadline: "Dialog 内只读审查与风险归纳",
    boundaryDetail: "作为 Dialog 的只读审查 lane 运行：默认不改文件、收紧审批，适合 review、回归检查和风险归纳。",
  ...
  dialog: {
    ...
    boundaryDetail: "适合派工、讨论、debug 和汇总；输入默认仍发给主 Agent，由它决定是否分派子会话。",
