...
export function PluginMarket({ onBack }: { onBack: () => void }) {
  ...
  // 获取内置插件列表（过滤掉「插件」和「设置」这类系统页面）
  const builtinPlugins = useMemo(() => {
    const systemViewIds = new Set(['plugins', 'settings'])
    return registry.getAll().filter((p) => !systemViewIds.has(p.viewId))
  }, [])
          ...
          <div className="space-y-2">
            {builtinPlugins.map((bp) => (
              <div
                ...
                className="flex items-center gap-3 p-3 rounded-lg bg-[var(--color-bg-secondary)] border border-[var(--color-border)] hover:border-orange-400/50 transition-colors cursor-pointer"
              >
