  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className="max-w-[120px] truncate">{preset?.name || currentModel}</span>
            ...
            {PRESET_MODELS.map((model) => (
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2 text-xs hover:bg-[var(--color-bg-hover)] transition-colors ${
                  currentModel === model.id ? 'text-[var(--color-accent)]' : 'text-[var(--color-text)]'
                }`}
              >
                <div>
                  <div className="font-medium">{model.name}</div>
                  <div className="text-[10px] text-[var(--color-text-secondary)]">{model.provider}</div>
                </div>
