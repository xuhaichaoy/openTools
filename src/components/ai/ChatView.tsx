...
import { Plus, Loader2, Bot, ArrowLeft, History, Square, ArrowDown, Download, Zap, Search, X } from "lucide-react";
      ...
      if (msg.role === "user") {
        ...
        if (msg.toolCalls?.length) {
          for (const tc of msg.toolCalls) {
            ...
            if (tc.result) lines.push(`> 结果: ${tc.result.slice(0, 200)}${tc.result.length > 200 ? "..." : ""}  `);
            lines.push("");
          }
        }
  ...
  return (
    <div className="flex h-full bg-[var(--color-bg)] relative">
      ...
      <div className="flex flex-col flex-1 min-w-0">
        ...
        {/* 消息区域 */}
        <div ref={messagesContainerRef} className="flex-1 overflow-y-auto px-2 py-2 space-y-6 scroll-smooth relative">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-[var(--color-text-secondary)]">
              <Bot className="w-14 h-14 mb-3 text-[var(--color-border)] opacity-50" />
              <p className="text-base font-medium opacity-50">有什么可以帮你的吗？</p>
          ...
          {messages.map((msg, idx) => {
            ...
            return (
              <div key={msg.id} className={searchQuery && !isSearchMatch ? "opacity-20 transition-opacity" : "transition-opacity"}>
                <MessageBubble
                  msg={msg}
        ...
        {isStreaming && (() => {
          ...
          const estimatedTokens = Math.ceil(
            [...(streamingMsg?.content || "")].reduce((sum, ch) => sum + (ch.charCodeAt(0) > 127 ? 1.5 : 0.25), 0)
          );
          return (
              ...
              {showTemplates && (
                <div className="absolute bottom-full mb-2 left-0 w-56 bg-[var(--color-bg)] border border-[var(--color-border)] rounded-xl shadow-xl overflow-hidden z-50">
                  <div className="px-3 py-2 border-b border-[var(--color-border)]">
                    <span className="text-[11px] font-medium text-[var(--color-text)]">快捷模板</span>
                  </div>
